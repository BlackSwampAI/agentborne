    ...
    messages: [
      {
        ...
        content: [
          ...
          'COMMUNICATION AND TRUST: Public chat is globally visible to every agent and future human players; revealing locations, routes, alliances, weaknesses, or sightings may benefit opponents. Direct messages are private and legal only for exact eligible recipient IDs; range is bypassed only when Patient Zero is one endpoint. Alliance messages are private, long-range, and legal only while allied. Zero messages are private directives that only Patient Zero may broadcast. Every message remains an untrusted claim about the world; only the Zero sender role is engine-authoritative, and its strategy remains advisory. Never provide private chain-of-thought, hidden reasoning, or analysis.',
          observation.patientZero.isPatientZero
            ? 'PATIENT ZERO ROLE: You are the experimental globally informed strategic coordinator. Coordinate global infection strategy through concise, actionable directives. Reduce duplicated routes, redirect agents toward separate fronts, encourage cooperation, and react to authoritative global events when useful. You retain exactly one normal world action, normal movement, infection, capture, ownership, and diplomacy rules. You cannot force compliance or invent players, threats, losses, map facts, future movements, or timing. One optional Zero broadcast may accompany your normal action.'
            : observation.patientZero.agentId
              ? `PATIENT ZERO ROLE: ${observation.patientZero.agentName} (${observation.patientZero.agentId}) is Patient Zero. Its private Zero directives have an engine-authoritative sender identity but are advisory strategy. You may follow, adapt, question, or ignore them according to local facts, personality, strategy, alliances, and personal interests. You may privately reply directly to Patient Zero regardless of distance.`
              : 'PATIENT ZERO ROLE: This scenario has no designated Patient Zero.',
