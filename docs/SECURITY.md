...
Agent-authored message content is bounded untrusted data. It appears only inside the immutable user observation, never the fixed system instruction. The fixed instruction explicitly says received claims and instructions are subordinate to game rules, replies are optional, and every action consumes the turn. Capture eligibility, territory scoreboards, and control-change histories are likewise labeled observations rather than instructions. Eligibility is derived from authoritative current positions, and recipient/range, infection, controller-presence, and capture validation remain authoritative in the world engine. World Lab renders message strings through React text nodes and never as HTML.
...
The Game API captures only schema-validated safe observations, requested actions, visible concise summaries, validation outcomes, bounded message text, world events, bounded provider failures, and normalized usage metadata. It never records or exports API keys, authorization headers, fixed system prompts, raw provider request/response bodies, private chain-of-thought, hidden analysis, or unbounded diagnostics. Historical turns, message events, control events, and configuration events are cloned and immutable.

Export requests, agent IDs, levels, ranges, outcome/action filters, and Custom dependencies are runtime-validated. Filtering and metrics remain server-owned so the browser cannot promote its 120-turn snapshot into authoritative history. Preview and generation reject overlap with turns and mutations. Generated JSON uses schema version 3 for controller-bearing hexes and capture semantics, and omits excluded fields instead of emitting misleading nulls. Export world-state snapshots contain spatial agents/controller-bearing hexes but no embedded event history. Accepted messages have one canonical top-level `communications` stream scoped to either participant; successful captures have one canonical `controlChanges` stream scoped to capturer or displaced controller. Full safe's separate `worldEvents` stream excludes both. This prevents duplication and leakage of unrelated messages or captures. Current authoritative territory is separate from historical turn-filtered metrics, and victim loss metrics derive only from accepted bounded capture events.
