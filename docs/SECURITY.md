...
The request uses only the selected model, messages, `max_tokens`, and `stream: false`. It deliberately sends no tools, `tool_choice`, `response_format`, `provider.require_parameters`, reasoning object, `reasoning_effort`, or model-specific parameter. Model IDs are never inspected or special-cased. Transport/provider failures, unavailable-model failures, text/JSON contract failures, and later simulation-rule rejection remain distinct safe outcomes. The adapter never silently substitutes a model or scripted behavior.
...
The model is explicitly instructed to return only one flat JSON decision with one concise visible summary and no hidden reasoning or chain-of-thought. The request does not configure or request reasoning; only numeric reasoning-token billing metadata is retained if OpenRouter reports it. The application stores no raw prompts, raw provider payloads, or private reasoning.
