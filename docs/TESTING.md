...
- Runtime mocks cover the explicit per-turn model, forced flat tool schema, required-parameter routing, metadata-derived reasoning, malformed/missing/duplicate/wrong calls, cancellation, output exhaustion, metadata preservation, and the unchanged one-request boundary without OpenRouter calls.
...
- Agent-runtime tests recursively inspect the flat model-agnostic OpenRouter tool schema for prohibited composition keywords, fixed trust instructions, forced one-call parsing, sentinel normalization, explicit model overrides, body-inclusive 75-second timeouts, cancellation, bounded diagnostics, secret non-leakage, and provider-name-independent mocked output.
...
World Lab also offers an explicit “Test selected model” probe. It uses the production tool contract, does not advance or mutate the world, may incur a small charge, and is never invoked by deterministic validation or CI.
