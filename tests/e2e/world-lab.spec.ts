...
test('runs the complete deterministic World Lab browser flow', async ({ page }) => {
  ...
  await expect(page.getByText(/H3 overlay ready · 61 cells · 6 agents/)).toBeVisible();
  ...
  await expect(page.getByText('2507bb46-7ae4-45ca-8dda-644c4f85ca14')).toBeVisible();
  ...
  await expect(page.getByText('Turn 2')).toBeVisible({ timeout: 10_000 });
  ...
  await expect(page.getByTestId('infected-count')).toHaveText('0 infected');
  await expect(page.getByText('Development world loaded with six agents.')).toBeVisible();
