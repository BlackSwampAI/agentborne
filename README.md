...
State is held only in the Game API process. The API captures one active safe experiment with up to 5,000 complete turn records while the browser snapshot remains capped at 120. Model and reasoning-profile assignments are part of schema-v7 exports and may be changed between requests; every mid-experiment change records its effective turn boundary. Schema-v6 exports remain importable, and older exports without reasoning profiles load as Provider default. Legacy schema-v5 imports retain their data but require an explicit model selection. A saved slug absent from the current compatible catalog is preserved and blocks execution until explicitly replaced. Every provider decision is one plain-text response containing a required flat JSON object with one world action, at most one communication, and at most one formal diplomacy intent. The runtime extracts and conservatively repairs JSON before strict local schemas and the world engine apply authoritative validation.
