...
Open the World Lab at <http://localhost:3000>. The Game API binds to <http://127.0.0.1:8787>; Next.js narrowly proxies `/api/game/*` to it. `OPENROUTER_API_KEY` is the only required OpenRouter environment value. Select a compatible global model in World Lab, then optionally override individual agents.
...
State is held only in the Game API process. The API captures one active safe experiment with up to 5,000 complete turn records while the browser snapshot remains capped at 120. Model assignments are part of schema-v6 exports and may be changed between requests; every mid-experiment change records its effective turn boundary. Legacy schema-v5 imports retain their data but require an explicit model selection. A saved slug absent from the current compatible catalog is preserved and blocks execution until explicitly replaced. Every provider decision is one forced, flat `submit_agent_decision` tool call containing one required world action, at most one communication, and at most one formal diplomacy intent; local schemas and the world engine remain authoritative.
